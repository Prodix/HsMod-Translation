...
namespace HsMod
{
    public static class PluginConfig
    {
        ...
        public static void ConfigBind(ConfigFile config)
        {
            ...
            //考虑导出单独配置
            skinCoin = config.Bind("Скины", "Монеты", -1, "ID предпочитаемого Lucky Coin, -1 означает отсутствие модификации (в игре аналог отсоединения может быть обновлен в реальном времени)");
            skinCardBack = config.Bind("Скины", "Обратная сторона", -1, "ID предпочитаемой обратной стороны карты, -1 означает отсутствие модификации (действует в реальном времени)");
            skinBoard = config.Bind("Скины", "Панель подбора", -1, "(тестовая функция, можно застрять в тавернах), Замена панели подбора, -1 означает отсутствие изменений");
            skinBgsBoard = config.Bind("Скины", "Боевая панель таверны", -1, "ID предпочитаемой боевой панели таверны, -1 означает отсутствие изменений");
            skinBgsFinisher = config.Bind("Скины", "Убийство в таверне", -1, "ID предпочитаемой убийств в таверне, -1 означает отсутствие изменений");
            skinBob = config.Bind("Скины", "Боб", -1, "ID предпочитаемого Боба, -1 означает, что изменения не производятся");
            isSkinDefalutHeroEnable = config.Bind("Скины", "Герой по умолчанию", false, "Замена всех скинов героев матча на скины по умолчанию.");
            skinHero = config.Bind("Скины", "Герои", -1, "(Использовать с осторожностью, не рекомендуется для неангажированных героев, приоритет ниже, чем у героев по умолчанию. Обычно рекомендуется загружать скин героя из файла и по F4 обновлять его после модификации; если вы снова в игре, вам также нужно будет симулировать выдергивание провода) ID предпочтений героя, -1 означает не модифицировать его");
            skinOpposingHero = config.Bind("Скины", "Герои противника", -1, "(Использовать с осторожностью, не рекомендуется для не висящих, приоритет ниже, чем у героя по умолчанию) ID предпочтений героя противника, -1 означает отсутствие модификации");
